...
function wireframe_invisible(game: Game, entity: Entity) {
    ...
    if (!wireframe) {
        let wireframe_entity = instantiate(game, [
            transform(),
            render_colored_unlit(game.MaterialColoredUnlitWireframe, game.MeshCube, [1, 0, 1, 1]),
        ]);
...
function wireframe_collider(game: Game, entity: Entity) {
    ...
    if (!wireframe) {
        let wireframe_entity = instantiate(game, [
            ...
            render_colored_unlit(game.MaterialColoredUnlitWireframe, game.MeshCube, [0, 1, 0, 1]),
        ]);
        wireframe = {
            entity: wireframe_entity,
