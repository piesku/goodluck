...
import {from_axis, get_axis_angle} from "../../common/quat.js";
...
function update(game: Game, entity: Entity) {
    ...
    if (control.Pitch && Math.abs(game.InputDelta["pad0_axis_4"]) > DEAD_ZONE) {
        ...
        let current_pitch = get_axis_angle(axis, transform.Rotation);
        ...
        if ((amount < 0 && current_pitch > -0.2) || (amount > 0 && current_pitch < Math.PI / 2.2)) {
            ...
            move.LocalRotations.push(from_axis([0, 0, 0, 0], AXIS_X, amount));
        }
    }
}
